...
Docta expone un servidor remoto [MCP (Model Context Protocol)](https://modelcontextprotocol.io) para que asistentes de IA consulten datos de bonos argentinos — rendimientos, duration, flujos de fondos y paridad — directamente desde la conversación.
...
El servidor expone 10 herramientas, una por cada endpoint documentado de la API de bonos:
...
| `compare_bonds` | (agrupa `yields/intraday`) | Comparación de 2–5 bonos en una tabla |
| `get_my_usage` | — | Tu plan y cuota restante (no consume requests) |

<Note>
  El alcance es **renta fija argentina**. La API no cubre acciones, CEDEARs ni futuros, y no
  expone precios, OHLC ni volumen operado: donde dice "intradiario" se trata de
  **rendimientos** intradiarios, no de cotizaciones.
</Note>
